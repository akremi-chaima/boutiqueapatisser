...
class UpdateBasketController extends AbstractController
{
    /* @var UserManager */
    private $userManager;
     ...
     * Add Order
     *
     * @Route("/api/update/basket/{userId}/{orderId}", methods={"PUT"})
     *
     * @OA\Tag(name="Orders")
     *
     * * @OA\RequestBody(
    ...
    {
        $order = $this->orderManager->findOneBy(['id' => $orderId]);
        if (is_null($order)) {
        ...
        $this->orderPastriesManager->deleteOrder($order);
